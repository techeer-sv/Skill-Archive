...
실제 기업 채용 프로세스를 기반으로 한 시뮬레이션 스킬셋. 채용 공고를 분석하여 **회사별 맞춤 프로세스**를 자동 구성합니다. ATS 스크리닝, FAANG 스타일 코딩 테스트 루브릭, 구조화 면접 평가를 경험할 수 있습니다.
...
| 코딩 테스트 | `/hiring-coding-test` | 4차원 루브릭 + Take-home 과제 |
| 면접 시뮬레이션 | `/hiring-interview` | 기술/인성/회사가치 면접 (6단계 Hire 스케일 + STAR) |
| 통합 파이프라인 | `/hiring-full-pipeline` | 회사별 맞춤 프로세스 (REJECT 시 즉시 중단) |
...
```
→ 이력서 + 공고 → 회사 자동 감지 → 맞춤 프로세스 구성 → 단계별 실행
...
| 회사 가치 면접 | 선택 가치 모두 2점+ AND 평균 3+ (1~5점 스케일) |
| Take-home 과제 | 4차원 모두 2점+ AND 평균 2.5+ (1~4점 스케일) |
...
skills/
  ...
  hiring-common/                    # 공통 모듈
    ...
    company-profiles.md             # Known Company DB
